...
from model_Feature_AAN_correct_audio_video_pooling import *
...
# Build dataloaders
def myDataloader(resnet_videoFeatures, flownet_videoFeatures, i3d_videoFeatures, audioFeatures_1, audioFeatures_2, args, shuffleBool=False):
    class my_dataset(Dataset):
        def __init__(self, resnet50_videoData, flownet_videoData, i3d_videoData, audioData_1, audioData_2):
        ...
        def __getitem__(self, index):
            return self.resnet50_videoData[index], self.flownet_videoData[index] ,self.i3d_videoData[index], self.audioData_1[index], self.audioData_2[index]

        def __len__(self):
            return len(self.i3d_videoData)

    # Build dataloaders
    my_dataloader = DataLoader(dataset=my_dataset(resnet_videoFeatures, flownet_videoFeatures, i3d_videoFeatures, audioFeatures_1, audioFeatures_2), batch_size=args.batch_size, shuffle=shuffleBool)
...
def validate_func(validate_loader, the_model, device):
    the_model.eval()
    all_cont_output = []
    for (v_resnet50_feature, v_flownet_feature, v_i3d_feature, v_audio_feature_1, v_audio_feature_2) in validate_loader:
        v_resnet50_feature, v_flownet_feature, v_i3d_feature, v_audio_feature_1, v_audio_feature_2 = \
            v_resnet50_feature.to(device), v_flownet_feature.to(device), v_i3d_feature.to(device), v_audio_feature_1.to(device), v_audio_feature_2.to(device)
        v_resnet50_feature, v_flownet_feature, v_i3d_feature, v_audio_feature_1, v_audio_feature_2 = \
            v_resnet50_feature.unsqueeze(1), v_flownet_feature.unsqueeze(1), v_i3d_feature.unsqueeze(1), v_audio_feature_1.unsqueeze(1), v_audio_feature_2.unsqueeze(1)

        vout = the_model(v_resnet50_feature, v_flownet_feature, v_i3d_feature, v_audio_feature_1, v_audio_feature_2)

        vout = vout.squeeze(1).cpu().detach().numpy()
        all_cont_output.append(vout)

        del v_resnet50_feature, v_flownet_feature, v_i3d_feature, v_audio_feature_1, v_audio_feature_2
        freeCacheMemory()
...
def loadingfiles():
    ...
    return resnet50_features, flownet_features, i3d_features, vggish_features, emobase_features
...
def main(args):
    ...
    # Get the input dim
    resnet50, flownet, i3d, vggish, emobase = loadingfiles()

    # Data loader
    all_dataset = myDataloader(resnet50, flownet, i3d, vggish, emobase, args, False)
...
if __name__ == "__main__":
    ...
    model_name = 'MDB_'+ emo_dim +'_COGNIMUSE_attention_model_Feature_AAN_video_audio.pth'
